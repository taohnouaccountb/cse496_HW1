...
if __name__ == '__main__':
    ...
    with tf.Session(config=config) as session:
        ...
        for epoch in range(FLAGS.max_epoch_num):
            ...
            print(str(conf_mxs))
            count_correct = 0
            count_sum = sum(sum(conf_mxs))
            for i in range(conf_mxs.shape[0]):
                count_correct += conf_mxs[i][i]
            test_ratio = count_correct * 1.0 / count_sum
            print("TEST CORRECT RATIO: ", test_ratio)
            ...
            if test_ratio > test_ratio_peak:
                ...
                path_prefix = saver.save(session, os.path.join(FLAGS.save_dir, file_name),
